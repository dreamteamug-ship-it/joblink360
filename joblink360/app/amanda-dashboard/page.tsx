  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {ventures.map((v) => (
          <div key={v.id} className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-sm hover:border-emerald-500/50 transition-all">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-lg font-medium tracking-tight uppercase">{v.name}</h2>
              <span className={`text-[10px] uppercase font-bold px-2 py-1 border border-current ${v.color}`}>{v.status}</span>
            </div>
            <div className="space-y-4">
              ...
              <div className="pt-4">
                <div className="w-full bg-zinc-800 h-[2px]">
                  <div className="bg-emerald-500 h-[2px]" style={{width: `${v.progress}%`}}></div>
                </div>
